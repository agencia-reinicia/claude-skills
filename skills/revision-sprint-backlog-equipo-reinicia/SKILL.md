...
Hay comportamientos que son **propios del modo desatendido** y NO aplican aquí (esta supervisada se ejecuta a demanda, con el PO delante), pero conviene conocerlos porque viven en la hermana: (1) **resiliencia de conector** — backoff/reintento antes de abortar y un 2º run de respaldo si el de las 06:00 cae; (2) **alcance acotado de Mejora 6** — en los pases **diarios** el refresco live de Col D se limita a filas con actividad o cuya resolución cambió, y el refresco del **100%** se reserva al pase semanal/cierre (aquí, al correr a demanda, se hace siempre completo); (3) **`routine_id`** en el reporte PASO 9 de Cliq. Todo ello documentado y versionado en la skill desatendida (v1.5+).
...
> 📌 **v3.10 — Robustez de match ANTES de decidir que hay rename.** Antes de concluir que un concepto del plan y un nombre de Data son cosas distintas:
...
> 3. **Detección SIMÉTRICA**: tras normalizar espacios, considera candidato a rename cuando el concepto del plan sea substring **o** superstring del nombre de la tarea — cubre tanto el sufijo añadido (`Modelo de Datos` → `Modelo de Datos [AICROV]`) como el **prefijo/cualificador** (`Implementación Modelo de Datos` ⊃ `Modelo de Datos`). (Caso real José f21 Sprint 7-26: el detector solo pillaba "ClickUp = Tiempos + sufijo" y se le escapó el prefijo → acabó sin resolver + huérfana duplicada.)
> 4. **Actuación (supervisado)**: el candidato así detectado se **propone al PO** con su propuesta/antipropuesta (siguiente sección) — nunca se aplica sin su validación.
...
#### Resolución producto→tarea (v3.8 — refunde y precede a las Mejoras 6 y 11)
...
🔁 **Ejecución OBLIGATORIA en cada pase (v3.8)**: la resolución `resolucion[]` y las **Mejoras 6 (Col D) y 11 (HYPERLINK) se ejecutan fila a fila en CADA pase**, leyendo ClickUp en vivo. ⛔ **PROHIBIDO "conservar los estatus/enlaces del pase previo"** como atajo: los conteos de la compuerta DoD miden lo ejecutado en **ESTE** pase. En el **primer pase de un sprint nuevo NO existe pase previo**. Saltarse esto y sellar igual (aunque se documente como "parcial") es el fallo del 29/05 que la compuerta existe para impedir.
...
⚡ **Patrón eficiente y resiliente para leer estatus en vivo (v3.9):** para refrescar Col D, **NO hacer N llamadas `clickup_get_task` una por tarea**. Traer el estado de todo el sprint en **lote** con `clickup_filter_tasks` filtrando por el **tag del sprint vigente** (p. ej. `sprint - 07 - 2026`), que devuelve todas las tareas vivas con su `status` de una sola vez, y resolver Col D contra ese mapa en memoria. Solo caer a `clickup_get_task` puntual para una tarea concreta que no aparezca en el filtro (p. ej. subtareas, o tareas sin el tag). Beneficio doble: muchas menos llamadas (más barato) y mucha **menos exposición a los 502 transitorios** de la API — validado en el piloto, donde sustituir `get_task` individuales por `filter_tasks` por tag absorbió los 502 sin abortar el pase.
...
##### 🚦 Compuerta de sellado — Definición de Hecho (DoD) OBLIGATORIA (v3.9 — por evidencia, sin sello parcial, invalida sello previo si falla)
...
1. **Releer literalmente** el PASO 6c (Resolución producto→tarea + Mejoras 6 y 11, incluida la regla de **ejecución obligatoria en cada pase**) y este PASO 8b antes de cerrar — NO de memoria. Caso real 29/05/2026: se dieron los estatus por "actualizados" mirando el snapshot del time entry en vez de escribir Col D desde ClickUp (Mejora 6), y los enlaces (Mejora 11) se omitieron por completo; el AUTOIA se selló igualmente y pareció cerrado sin estarlo. Reincidencia en el piloto del 31/05: se selló en "parcial" conservando estatus/enlaces de un pase previo inexistente.
2. **Recomputar los conteos reales** desde el sheet y el artefacto `resolucion[]` **construido en este pase** (una fila "conservada del pase previo" NO cuenta como ejecutada): `n_plan`, `n_resueltas`, `n_drifts`, `n_resolubles`, `n_enlazadas`, `n_ambiguas`.
...
4. **Regla dura — NO existe "sello parcial":** o (a)–(d) están **TODOS en ✅** y se escribe el sello (e), o **NO se escribe E1/M1** y el pase queda **sin cerrar**. Queda **PROHIBIDO** escribir el sello si **(a) o (a-bis) están por debajo del 100%**, aunque se documente como "parcial": documentarlo NO autoriza a sellar. Como Mejora 6 y 11 son deterministas, lo correcto es **ejecutarlas** (no requieren confirmación del PO) y volver a emitir el checklist; solo si aun así no se completan, se deja sin cerrar y se reporta al PO. Registrar el resultado en el Log de Cambios (`DOD_CIERRE`) con los conteos reales.

5. **Sello previo cuando la DoD falla (v3.9):** si la compuerta NO pasa pero el AUTOIA **ya tenía un sello previo** (de un pase anterior) en `Tiempos!E1` / `Log!M1`, ese sello queda **desfasado y contradictorio** con el estado real (sin cerrar). En ese caso **no basta con no reescribirlo**: hay que **invalidarlo** para que el fichero no quede mintiendo "cerrado". Procedimiento: sustituir el contenido de `E1`/`M1` por una marca explícita **`NO CERRADA — [fecha/hora] — DoD incompleta`** (no dejar el timestamp viejo), y registrar `DOD_CIERRE` en el Log con el motivo. En supervisado, además, **avisar al PO** de que se ha limpiado un sello previo desfasado. Así el siguiente pase parte de un estado honesto y no de un "cerrado" falso heredado.
