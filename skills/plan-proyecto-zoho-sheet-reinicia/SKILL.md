...
> ⚠️ **Los números de fila-título de abajo son ORIENTATIVOS (plantilla recién copiada) y NO se dan por buenos: se releen EN VIVO (§8.1 paso 1) antes de tocar nada.** La plantilla puede traer más/menos placeholders y cualquier inserción previa los desplaza (incidente real: SOPORTE ACTIVO apareció en la fila 36, no en la 32). Localizar cada sección **por el texto de su fila-título** (`PLAN …`), nunca por índice fijo.

| Sección / Tabla | Fuente ClickUp | Filtro | Fila-título (orientativa) |
|---|---|---|---|
...
- **Los meses se repiten en la fila-título de CADA sección.** En planes **EN**, traducirlos **por sección** (no basta con traducir la primera).
...
## 8. Inserción de filas en las tablas del Plan (PROTOCOLO OBLIGATORIO)
...
### 8.0 INVARIANTES — NUNCA (violarlos corrompe la estructura; incidente real: se aplastó la cabecera de "PLAN SOPORTE ACTIVO" al insertar sobre su fila-título)
- **NUNCA** escribir en la **fila-título** de una sección (`PLAN IMPLEMENTACIÓN` / `PLAN SOPORTE ACTIVO` / `PLAN SOPORTE CERRADO` / `RESUMEN HISTÓRICO`) ni en su **fila-cabecera** (la de `Épica | Tipo Producto | Mes | …`).
- **NUNCA** escribir en las **2 últimas filas de datos** de una tabla: son el **buffer reservado** (la última lleva el borde de cierre; la penúltima arrastra el formato + desplegable de Estatus + semáforo). Se dejan **siempre vacías**.
- **NUNCA** insertar con `row` apuntando a una fila-título o a una fila-cabecera. El **ancla de inserción** es **una fila de datos INTERIOR** de la sección en curso (= `fila_título_siguiente − 2`).
- **NUNCA** usar números de fila memorizados, del resumen de contexto, de la memoria o de versiones anteriores de la skill (p. ej. "títulos en 7/32/57/81"): la plantilla puede traer más/menos placeholders y las inserciones previas los desplazan. Los límites se **releen EN VIVO** en cada ejecución.
- **NUNCA localizar títulos/cabeceras contando filas de una lectura por rango.** `worksheet.content.get` en modo `array` **COLAPSA las filas vacías**, así que el índice del array **NO equivale al número de fila** (incidente real: "PLAN SOPORTE CERRADO" pareció estar en la fila 63 contando el array cuando en realidad estaba en la 59). Las secciones se localizan **SIEMPRE con `ZohoSheet_find`** (devuelve el `row_index` REAL); las lecturas por rango solo valen para leer el CONTENIDO de filas **ya localizadas**, nunca para ubicarlas. Para verificar una celda concreta en una aserción, usar **`get_content_of_cell`** (fiable), no el array.

### 8.1 Protocolo gated (5 pasos) — obligatorio ANTES de escribir cualquier sección
Orden estricto, sin saltarse ningún paso: **LOCALIZAR (con `find`) → CALCULAR → INSERTAR → VERIFICAR → ESCRIBIR.**

1. **LOCALIZAR con `find` (no contando filas).** Localizar las **4 filas-título** con **`ZohoSheet_find`** (`scope: worksheet`, una búsqueda por título: `PLAN IMPLEMENTACIÓN`, `PLAN SOPORTE ACTIVO`, `PLAN SOPORTE CERRADO`, `RESUMEN HISTÓRICO`) → cada una devuelve su `row_index` **REAL**; la fila-cabecera es la inmediatamente posterior. **Nunca** contar filas de una lectura por rango (el array colapsa vacías; índice ≠ fila). Recalcular TODOS los rangos desde estos `row_index`. Para leer el CONTENIDO de filas ya localizadas (p. ej. replicar una cabecera), sí vale `worksheet.content.get` acotado a esas filas concretas.
2. **CALCULAR.** Para cada sección, **rango escribible = `[fila_cabecera + 1 … fila_título_siguiente − 3]`** (las filas `−2` y `−1` quedan como buffer). Contar por adelantado **cuántas filas necesita TODA la sección** (todos sus productos) y cuántas caben en ese rango. Para la última sección (RESUMEN HISTÓRICO) el límite superior es `used_row − 2` o el final lógico de su tabla.
3. **INSERTAR (solo si faltan).** Si `necesarias > caben`, insertar el **déficit completo, de una sola vez y por adelantado**, con `row = fila_título_siguiente − 2` (ancla interior), **1 fila por llamada**, y **replicado en AMBAS pestañas** (40# y 41#) al mismo índice para no desincronizarlas. Jamás insertar en la fila-título.
4. **VERIFICAR (aserción dura — bloqueante).** **Re-leer** columnas A–B y comprobar que **siguen existiendo los 4 títulos y sus 4 cabeceras**, ahora desplazados exactamente tantas filas como se insertaron por encima. Comprobar el **recuento** real de filas insertadas. **Si falta algún título/cabecera, o el recuento no cuadra, o los desplazamientos no coinciden → ABORTAR: no escribir contenido, dejar constancia y reparar antes de continuar.**
5. **ESCRIBIR — con GUARDA DE LÍMITE POR LOTE (aserción por escritura).** Antes de CADA `cells.content.set`, comprobar que **todas** las filas destino del lote caen dentro del **rango escribible** `[cabecera+1 … título_siguiente−3]` de su sección. Si **una sola** fila se sale → **NO escribir el lote**: volver a INSERTAR (paso 3) y recalcular. Esta guarda es **por cada escritura**, no solo el cálculo previo del paso CALCULAR — es lo que evita **desbordar una tabla sobre el título/cabecera de la siguiente** (incidente real: IMPLANTACIÓN se escribió sobre el título+cabecera de SOPORTE ACTIVO, mezclando las tablas). Si una fila-límite es dudosa, confirmarla con **`get_content_of_cell`** antes de escribir.

### 8.2 Chequeo de integridad estructural (rutina reutilizable)
Ejecutar **antes y después** de CUALQUIER operación estructural (inserción §8, **borrado** de filas de Formación Interna, o movimientos de hito que impliquen filas):
- Localizar y contar los títulos con **`ZohoSheet_find`** (una búsqueda por título, `scope: worksheet` en cada pestaña, o `scope: workbook`): deben aparecer **exactamente 4** (`PLAN IMPLEMENTACIÓN`, `PLAN SOPORTE ACTIVO`, `PLAN SOPORTE CERRADO`, `RESUMEN HISTÓRICO`) en cada pestaña. Si un título falta (`matches_found` de menos) → estructura rota.
- Cada título debe tener **inmediatamente debajo su fila-cabecera NO vacía** — comprobar la celda `B(título+1)` con **`get_content_of_cell`** (debe decir `Épica`).
- Si el conteo ≠ 4 **o** alguna cabecera está vacía → la estructura está rota → **ABORTAR** y reparar (recrear título+cabecera replicando los de una sección intacta, con su fila de meses y sus semanas 1–24) antes de seguir.

**Regla (resumen):** cuando en una tabla quedan pocas filas libres dentro del cuerpo formateado y aún hay contenido por meter, **primero se insertan las filas necesarias en una fila de datos INTERIOR (ancla = `título_siguiente − 2`), respetando el buffer de 2 filas finales; solo después se pega el contenido.** Nunca al revés, nunca sobre título/cabecera, y nunca escribir más allá del cuerpo formateado sin insertar antes.

Razones y detalles:
- **Por qué el buffer de 2 filas finales**: la última fila del cuerpo suele llevar el borde de cierre de la tabla; la penúltima arrastra formato + desplegable (Estatus) + formato condicional (semáforo). Insertar **por encima de ese buffer** (ancla interior) hace que las filas nuevas hereden ese formato y la tabla conserve su fila de cierre. Escribir sobre esas 2 filas rompe el cierre y el desplegable.
...
- `ZohoSheet_insert_row` inserta **una fila por llamada** (param `row` = índice); **desplaza hacia abajo todo lo que haya en ese índice o por debajo** (por eso insertar sobre la fila-título la empuja/aplasta). Para N filas, **N llamadas** al mismo índice interior, y **verificar el recuento con una lectura**.
- **Tras insertar, recalcular posiciones de TODAS las tablas inferiores** releyendo (no calculando a ciegas): se desplazan hacia abajo tantas filas como se hayan insertado. Recalcular también las **filas-título/meses** de las secciones inferiores y, en la cara cliente (40#), revisar que la **rejilla del Gantt y los hitos** sigan alineados; **repintar calendario/Gantt** si hizo falta.
- **El borrado de filas** (p. ej. Formación Interna, §PASO 3) es también estructura → aplicar el mismo chequeo de integridad (8.2) antes y después, hacerlo **al final de la fase de contenido** y **repintar** después.
- La región con formato de la plantilla es **más corta** que el nº habitual de productos, por lo que este protocolo aplica casi siempre que se puebla un Plan real. El nº de placeholders **se lee en vivo** (paso 8.1.1), no se asume.
